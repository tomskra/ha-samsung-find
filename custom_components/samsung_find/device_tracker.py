...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    for device in devices:
        if 'subType' in device['data'] and device['data']['subType'] == 'CANAL2':
            entities += [SamsungDeviceTracker(hass, coordinator, device, "left")]
            entities += [SamsungDeviceTracker(hass, coordinator, device, "right")]
        entities += [SamsungDeviceTracker(hass, coordinator, device)]
